...
if __name__ == '__main__':
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=4) as executor:
        futures = executor.map(compute_cycled_money, all_accounts, chunksize=100)
        for account, cycled_amt in tqdm(futures, total=total):
            cycled_money[account] = cycled_amt
        # for account, cycled_amt in tqdm(zip(all_accounts, futures), total=total):
        #     cycled_money[account] = cycled_amt
