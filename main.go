...
// encryptAndPack: signs payload, encrypts with AES-GCM + RSA-OAEP envelope
...
// accountID generates deterministic account IDs.
// Prefix alternates between "000_" and "001_" to cover both bank prefixes.
...
func opTransfer(
	...
	payload := types.SecureMessage{
		...
		From:      from,
		To:        to,
		Amount:    amount.Int64() + 1, // avoid zero
...
func worker(
	...
	for {
		...
		switch mode {
		case "create":
			// Sequential account creation — each worker picks a unique slot via atomic counter
			idx := int(ops.Add(1))
			id := accountID(idx)
		...
		case "mixed":
			// TPC-C-inspired mix: 88% transfer, 8% create, 4% read (we model read as a 0-amount self-check transfer)
			r := mathrand.Intn(100)
			switch {
			case r < 88:
				opType = "transfer"
				from := accountID(mathrand.Intn(numAccounts))
				to := accountID(mathrand.Intn(numAccounts))
				for to == from { to = accountID(mathrand.Intn(numAccounts)) }
				ok, err = opTransfer(ctx, client, senderPriv, receiverPub, from, to)
			default:
				opType = "create_account"
				idx := int(ops.Add(1)) + numAccounts
				ok, err = opCreateAccount(ctx, client, senderPriv, receiverPub, accountID(idx))
			}
...
func main() {
	...
	fmt.Printf("Target       : %s\n", *flagTarget)
	fmt.Printf("Account pool : %d\n", *flagAccounts)
	fmt.Println()

	// Load keys
	senderPrivBytes, err := os.ReadFile(*flagSenderKey)
	if err != nil { panic(fmt.Sprintf("Cannot read sender key: %v", err)) }
	senderPriv := parsePrivateKey(string(senderPrivBytes))

	receiverPubBytes, err := os.ReadFile(*flagReceiverKey)
	if err != nil { panic(fmt.Sprintf("Cannot read receiver key: %v", err)) }
	receiverPub := parsePublicKey(receiverPubBytes)

	// Connect (shared connection pool via gRPC)
	conn, err := grpc.Dial(*flagTarget,
		grpc.WithTransportCredentials(insecure.NewCredentials()),
		grpc.WithBlock(),
		grpc.WithTimeout(10*time.Second),
	)
	if err != nil { panic(fmt.Sprintf("Cannot connect: %v", err)) }
	defer conn.Close()

	client := pb.NewSecurityServiceClient(conn)

	// ── PHASE 0: Seed accounts (if transfer mode needs them) ──
	if *flagMode == "transfer" || *flagMode == "mixed" {
		fmt.Printf("Seeding %d accounts...\n", *flagAccounts)
		seedCtx, cancel := context.WithTimeout(context.Background(), 120*time.Second)
		defer cancel()

		var seeded, failed int
		for i := 0; i < *flagAccounts; i++ {
			ok, err := opCreateAccount(seedCtx, client, senderPriv, receiverPub, accountID(i))
			if err != nil || !ok { failed++ } else { seeded++ }
			if i%10 == 0 { fmt.Printf("\r  Seeded %d/%d (failed: %d)", seeded, *flagAccounts, failed) }
		}
		fmt.Printf("\r  Done: %d seeded, %d failed\n\n", seeded, failed)
	}
	...
	for i := 0; i < *flagConcurrency; i++ {
		...
		go func() {
			...
			worker(ctx, stop, warmupUntil, client, senderPriv, receiverPub,
				*flagAccounts, *flagMode, stats, &ops)
		}()
	}
